...
const props = defineProps<{ alerte: AlertMaintenance }>();
</script>
<template>
    <div
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                    <p class="text-sm font-semibold text-gray-900 truncate">
                        {{ props.alerte.engin?.designation }}
                    </p>
                    <span class="px-2 py-0.5 text-[10px] font-medium rounded bg-yellow-100 text-yellow-700 capitalize">
                        {{ props.alerte.maintenance_automatique?.type }}
                    </span>
                </div>
                <p class="text-xs text-gray-600 mt-1">Numéro série : <span class="font-semibold text-gray-900">
                        {{ props.alerte.engin.numero_serie }}
                    </span>
                </p>
                <p class="text-xs text-gray-600 mt-1">
                    Seuil atteint :
                    <span class="font-semibold text-gray-900">
                        {{ props.alerte.maintenance_automatique?.type === 'horaire'
                            ? props.alerte.compteur_declencheur + ' h'
                            : props.alerte.compteur_declencheur + ' km' }}
                    </span>
                </p>
